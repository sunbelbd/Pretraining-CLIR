...
def paddle_masked_fill(data_to_fill, filling_mask, fill_value):
    """
    :return: a new tensor with filling_mask==1 position filled by fill_value
    """
    assert data_to_fill.shape == filling_mask.shape
    fill_tensor = paddle.full_like(data_to_fill, fill_value)
    return paddle.where(filling_mask == 1, fill_tensor, data_to_fill)
